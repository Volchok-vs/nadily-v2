/**
 * Модуль генерації аналітичних звітів (Мобільне завантаження + А4)
 * Зберігає повну логіку автономної модалки, фільтрації та експорту.
 */
...
function initAnalyticsModule() {
    ...
    // Очищаємо інлайнові onclick, щоб уникнути конфліктів асинхронності
    btnGenerate.removeAttribute('onclick'); 

    // Прив'язуємо запуск модалки до кліку на кнопку
...
async function runAnalyticsModalFlow() {
    ...
    async function generateAggregatedReport(startDate, endDate) {
        ...
        try {
            ...
            // Інтегруємо html2pdf.js всередину вікна звіту
            let reportHtml = `
<!DOCTYPE html>
<html lang="uk">
<head>
    ...
    <!-- Підключення бібліотеки генерації PDF прямо з CDN -->
    ...
    <script>
        function downloadReportPDF() {
            ...
            // Візуальний фідбек завантаження
            const originalText = btn.innerHTML;
            btn.innerHTML = '⏳ Генерація PDF...';
            btn.disabled = true;

            const opt = {
                margin:       [12, 15, 12, 15], // відступи А4
                filename:     'report_${fileNameDate}.pdf',
                image:        { type: 'jpeg', quality: 0.98 },
                html2canvas:  { scale: 2, useCORS: true, letterRendering: true },
                jsPDF:        { unit: 'mm', format: 'a4', orientation: 'portrait' }
            };

            // Запускаємо рендеринг у PDF та зберігаємо
            html2pdf().set(opt).from(element).save().then(() => {
                ...
                btn.innerHTML = originalText;
                btn.disabled = false;
                alert('Не вдалося згенерувати PDF через внутрішню помилку скрипта.');
            });
        }
    <\/script>
</head>
