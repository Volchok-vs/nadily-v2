...
function attachEvents(menu, map, layers) {
    ...
    const apply = () => {
        ...
        const isAll = menu.querySelector('#chk-all').checked;
        const isFreeMode = menu.querySelector('#chk-free').checked;
        const isTakenMode = menu.querySelector('#chk-taken').checked;
        const isMineMode = menu.querySelector('#chk-mine').checked;
    ...
    searchInput.onfocus = () => { if (!menu.querySelector('#chk-all').checked) switchMode('chk-all'); };
    searchInput.oninput = apply;
    menu.querySelector('#chk-all').onclick = () => switchMode('chk-all');
    menu.querySelector('#chk-free').onclick = () => switchMode('chk-free');

    if (menu.querySelector('#chk-taken')) menu.querySelector('#chk-taken').onclick = () => switchMode('chk-taken');
    if (menu.querySelector('#chk-mine')) menu.querySelector('#chk-mine').onclick = () => switchMode('chk-mine');
    ...
    if (cb) cb.onchange = (e) => { config.showBusiness = e.target.checked; initSearchAndFilters(map, layers); };
    if (cv) cv.onchange = (e) => { config.showVillage = e.target.checked; initSearchAndFilters(map, layers); };
    if (!map._searchEventAttached) {
        map.on('click', (e) => {
            // Перевіряємо, чи в полі пошуку щось є
            if (searchInput.value.trim() !== "") {
                searchInput.value = "";
                // Викликаємо функцію apply, яку ви визначили вище, 
                // щоб карта оновилася (показала всі дільниці)
                apply(); 
...
export function initSearchAndFilters(map, allParcelLayers) {
    ...
    menu.innerHTML = `
        ...
        <div class="filter-section" id="sec-mine" style="display:none;">
            <label class="row"><input type="checkbox" id="chk-mine"> <span style="margin-left:10px;">👤 Мої дільниці</span> <span class="count-badge">0</span></label>
